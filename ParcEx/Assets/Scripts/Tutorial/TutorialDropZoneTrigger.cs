...
namespace Delivery
{
    public class TutorialDropZoneTrigger : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            Disable();
            TutorialDeliveryManager._onDeliverySuccessAccept.AddListener((BaseDelivery del) =>
            {
            ...
            TutorialDeliveryManager._onPackagePickup.AddListener((BaseDelivery del) => Enable(del));
            TutorialDeliveryManager._onDeliveryFail.AddListener((BaseDelivery del) => Disable());
        }
        ...
        private void OnTriggerStay(Collider other)
        {
            if (other.name == address)
            {
                if (hasStayed)
                {
                    ...
                    {
                        TutorialDeliveryManager._onDeliveryCompleted.Invoke(deliveryForZone);
