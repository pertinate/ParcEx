...
public class UIManager : MonoBehaviour {
    ...
    private void Awake()
    {
        ...
        DeliveryDatabase.onPendDelAdd.AddListener((BaseDelivery del) => HandleNewDelivery(del));
        DeliveryManager.onDeliverySuccessAccept.AddListener((BaseDelivery del) => HandleUIOnDeliveryUpdate(true, del));
        DeliveryManager.onDeliveryDecline.AddListener((BaseDelivery del) => HandleUIOnDeliveryUpdate(false, del));
        ...
        DeliveryManager.onDeliveryCompleted.AddListener((BaseDelivery del) =>
        {
            ...
            foreach(Transform t in go.transform)
            {
                switch (t.name)
                {
                    ...
                    case "Time Until Expire":
                        ...
                        text.text = "Expires in: " + del.timeUntilExpire + " seconds";
                        StartCoroutine(HandleExpiredText(text, del));
                        break;
                    case "Reward":
                        t.GetComponent<Text>().text = "Reward: $" + del.cashReward.ToString();
                        break;
                    case "Accept"://button
                        t.GetComponent<Button>().onClick.AddListener(() => DeliveryManager.onDeliveryAccept.Invoke(del));
                        break;
                    case "Decline"://button
                        t.GetComponent<Button>().onClick.AddListener(() => DeliveryManager.onDeliveryDecline.Invoke(del));
                        break;
                }
            }
    ...
    {
        DeliveryManager.onDeliveryExpire.Invoke(del);
            ...
            foreach (Transform t in go.transform)
            {
                switch (t.name)
                {
                    case "Address":
                        ...
                        break;
                    case "Time to Complete":
                        StartCoroutine(HandleRunOutOfTime(t.GetComponent<Text>(), del));
                        break;
                    case "Reward":
                        t.GetComponent<Text>().text = "Reward: " + del.cashReward;
                        break;
                }
            }
            DeliveryDatabase.instance.currentDeliveryCount--;
            for (int i = 0; i < pendingList.Count; i++)
            {
                GameObject test;
                if (pendingList.TryGetValue(del, out test))
    ...
    private IEnumerator HandleRunOutOfTime(Text t, BaseDelivery del)
    {
        TimeSpan ts = TimeSpan.FromSeconds(del.timeLeft);
        t.text = string.Format("Time remaining: {0} minutes and {1} seconds", ts.Minutes, ts.Seconds);
        while (del.timeLeft > 0)
        {
            yield return new WaitForSeconds(1f);
            del.timeLeft--;
            ts = TimeSpan.FromSeconds(del.timeLeft);
            t.text = string.Format("Time remaining: {0} minutes and {1} seconds", ts.Minutes, ts.Seconds);
        }
        if(del.timeLeft <= 0)
        {
            DeliveryManager.onDeliveryFail.Invoke(del);
            for (int i = 0; i < acceptedList.Count; i++)
            {
                GameObject test;
                if(acceptedList.TryGetValue(del, out test))
                {
                    Destroy(test);
                    acceptedList.Remove(del);
                }
            }
        }
    }
